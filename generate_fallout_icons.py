#!/usr/bin/env python3
# ============================================================================
# FILE:    generate_fallout_icons.py
# TASK:    Build the "BeautyLine-Fallout" icon theme (folder/file icons for
#          the Omarchy Fallout theme).
# STATUS:  DONE
# OWNER:   agent A 2026-09-09
#
# WHAT THIS FILE DOES:
#   Adapted from omarchy-matrix-rain's generate_matrix_icons.py (same repo
#   author, same technique) — only the folder-gradient colors and output
#   names changed, the SVG recolor/black-fade/symlink logic is unchanged.
#   Unlike matrix's DOWNLOAD_FOLDER_GREEN (a fixed asset-matched color, not
#   theme-derived), this theme's folder gradient IS derived straight from
#   colors.toml (light_foreground -> darker_background) since our whole
#   palette is already the wasteland/amber family — no reason to special-
#   case away from it here.
#
...
# "BeautyLine-Fallout", covering everything a file manager can show:
#
...
DEFAULT_DST = Path.home() / ".local" / "share" / "icons" / "BeautyLine-Fallout"
...
# Fully theme-derived (unlike matrix's fixed DOWNLOAD_FOLDER_GREEN): this
# palette is already the wasteland/amber family end to end, so the folder
# gradient just reads light_foreground -> darker_background straight out of
# colors.toml. Fallback constants below match this theme's own colors.toml
# in case the file isn't found (kept in sync by hand — small, static file).
FOLDER_TOP_DEFAULT = "#F2DDB0"      # light_foreground
FOLDER_BOTTOM_DEFAULT = "#050403"  # darker_background
...
def colors_from_theme(theme_name):
    ...
    light_fg = data.get("light_foreground")
    if not darker_bg or not light_fg:
        return None
    return light_fg, darker_bg
...
def write_index_theme(dst):
    (dst / "index.theme").write_text(
        "[Icon Theme]\n"
        "Name=BeautyLine-Fallout\n"
        "Comment=BeautyLine with wasteland-amber folders and black-fade file/device icons\n"
...
def main():
    ...
    p.add_argument("--theme-name", default="fallout", help="Omarchy theme folder to pull colors from")
    ...
    if top is None or bottom is None:
        ...
        top = top or (derived[0] if derived else FOLDER_TOP_DEFAULT)
        bottom = bottom or (derived[1] if derived else FOLDER_BOTTOM_DEFAULT)
